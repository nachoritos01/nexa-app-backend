...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .terms { background-color: #fef3c7; padding: 15px; border-radius: 8px; font-size: 11px; }
...
<body>
    @php
        ...
        $discountAmount = $quote->subtotal * ($quote->discount / 100);
    @endphp

    <div class="header">
        <div class="doc-number">
            <h2>Cotización {{ $quote->number }}</h2>
            <p>Fecha: {{ $quote->date }}</p>
            <p>Válida hasta: {{ $quote->valid_until }}</p>
    ...
    <div class="section">
        <div class="totals">
            <table>
                <tr><td class="label">Subtotal:</td><td class="amount">${{ number_format($quote->subtotal, 2) }} {{ $currency }}</td></tr>
                @if($quote->discount > 0)
                <tr><td class="label">Descuento ({{ $quote->discount }}%):</td><td class="amount">-${{ number_format($discountAmount, 2) }} {{ $currency }}</td></tr>
                @endif
                @if($quote->tax > 0)
                <tr><td class="label">Impuesto ({{ $quote->tax }}%):</td><td class="amount">${{ number_format(($quote->subtotal - $discountAmount) * ($quote->tax / 100), 2) }} {{ $currency }}</td></tr>
                @endif
                <tr class="total-row"><td class="label">TOTAL:</td><td class="amount">${{ number_format($quote->total, 2) }} {{ $currency }}</td></tr>
            </table>
        </div>
    </div>
